...
- When a sales, investor, or product pitch needs stronger belief progression and audience alignment.
- When the pitch must move from attention to trust to commitment with less resistance.
- When existing pitch materials open with features instead of the audience's world.
- When the audience type (technical, executive, consumer, skeptical) requires different sequencing.

## When NOT to Use

- For internal status updates or informational presentations (no persuasion objective).
- When the audience already has full conviction — move to objection handling instead.
- For academic papers or neutral analysis reports.

## Combines with

- `customer-psychographic-profiler` — build the audience profile before designing the arc.
- `price-psychology-strategist` — structure the offer and pricing presentation after the arc is set.
- `copywriting-psychologist` — write the actual slide/deck copy following the arc.

## Workflow

1. Establish audience type, awareness stage, and pitch format (deck, talk track, one-pager).
2. Define the single decision the pitch must produce.
3. Write the audience's world and stakes — no product mention yet.
4. Build the desire arc: better future → cost of inaction → contrast.
5. Introduce the solution as the natural bridge, then close with proof + clear next step.

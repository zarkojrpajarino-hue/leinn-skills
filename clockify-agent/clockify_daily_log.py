#!/usr/bin/env python3
"""
clockify_daily_log.py  v2

Reads Claude Code session JSONL files, groups work into blocks,
generates descriptions via Claude Haiku, asks for manual entries,
shows complete day table, then confirms each entry.

Usage:
    python clockify_daily_log.py
    python clockify_daily_log.py --yesterday
    python clockify_daily_log.py --dry-run
    python clockify_daily_log.py --date 2026-05-23

Setup:
    1. Copy clockify_config.example.json -> clockify_config.json
    2. Fill in your API keys (see SKILL.md)
"""
...
def _load_config():
    ...
    if not config_path.exists():
        raise FileNotFoundError(
            "\nConfig not found: " + str(config_path) +
                ...
                for line in f:
                    try:
                        ...
                        if text.startswith(("<task-notification>", "<system-reminder>")):
                            continue
                        msgs.append({"role": t, "timestamp": dt, "text": text[:400]})
                    except Exception:
                        continue
        except Exception:
            continue
        if msgs:
            sessions.append((jsonl_file.stem[:8], sorted(msgs, key=lambda x: x["timestamp"])))
    return sessions
...
def generate_description_multi(all_messages, n_sessions):
    ...
    for m in all_messages[:25]:
        ...
        lines.append(role + ": " + m["text"][:150])

    context = (
        "These messages come from " + str(n_sessions) + " parallel Claude Code sessions.\n\n"
    ...
    prompt = (
        ...
        "'optimus k, Phase 93 auth RLS JWT' / 'leinn pitch, one-pager HTML'. "
    ...
    try:
        ...
        print("  Claude API error " + str(r.status_code))
...
def ask_manual_entries(target_d, offset_hours):
    W = 60
    print()
    print("-" * W)
    print("  Hay trabajo que anadir fuera de las sesiones?")
    print("  (reuniones, llamadas, trabajo sin ordenador...)")
    print()
    print("  Ejemplos:")
    print("    reunion con inversores 10:00-11:30")
    print("    llamada cliente 15:00-16:00, gym 07:00-08:00")
    print("    clase LEINN toda la manana 9:00-14:00")
    print()
    ...
    n = len(entries)
    if n:
        print("OK -- " + str(n) + " entrada" + ("s" if n != 1 else "") + " detectada" + ("s" if n != 1 else ""))
    else:
        print("no se detectaron entradas validas")
    return entries


def _parse_manual_with_claude(text, target_d, offset_hours):
    prompt = (
        "Parse these work/time entries. Today: " + str(target_d) + ". "
        "Local timezone: UTC+" + str(offset_hours) + ".\n\n"
        "User input: \"" + text + "\"\n\n"
        ...
        "Example: [{\"description\": \"reunion con inversores\", \"start_local\": \"10:00\", \"end_local\": \"11:30\"}]"
    )
    try:
        r = requests.post(
            "https://api.anthropic.com/v1/messages",
        ...
        if r.status_code != 200:
            print("\n  Claude API error " + str(r.status_code))
            return []
...
def _fmt_dur(start_utc, end_utc):
    ...
    return str(h) + "h" + str(m).zfill(2) + "m", mins
...
def print_day_table(entries, offset_hours, title):
    ...
    print("  " + title)
    print("=" * W)
    print("  " + " #".rjust(2) + "  " + "Inicio".rjust(5) + "  " + "Fin".rjust(5) +
          "  " + "Dur".rjust(6) + "  " + "Tipo".ljust(8) + "  Descripcion")
    print("-" * W)
    ...
    for i, e in enumerate(entries, 1):
        ...
        print("  " + str(i).rjust(2) + "  " + sl.strftime("%H:%M").rjust(5) +
              "  " + el.strftime("%H:%M").rjust(5) + "  " + dur.rjust(6) +
              "  " + tipo + "  " + desc + warn)

    print("-" * W)
    th, tm = int(total_mins) // 60, int(total_mins) % 60
    n = len(entries)
    print("  Total: " + str(th) + "h " + str(tm).zfill(2) + "m  (" +
          str(n) + " entrada" + ("s" if n != 1 else "") + ")")
    print("=" * W)
...
def check_blocked_window(start_utc, end_utc):
    ...
    for win in BLOCKED_WINDOWS:
        ...
        if start_local < we and end_local > ws:
            return "overlaps " + wd + " blocked window " + str(sh).zfill(2) + ":" + str(sm).zfill(2) + "-" + str(eh).zfill(2) + ":" + str(em).zfill(2)
    return None
...
def get_existing_entries_for_day(target_d):
    ...
    try:
        r = requests.get(
            CLOCKIFY_URL + "/workspaces/" + CLOCKIFY_WORKSPACE + "/user/" + uid + "/time-entries",
            headers={"X-Api-Key": CLOCKIFY_API_KEY},
            params={
...
def create_entry(start_utc, end_utc, description):
    r = requests.post(
        CLOCKIFY_URL + "/workspaces/" + CLOCKIFY_WORKSPACE + "/time-entries",
...
def main():
    ...
    print()
    print("Clockify Daily Log -- " + str(target) + " (" + label + ")")
    print("=" * 60)

    sessions = load_messages_per_session(target)
    if not sessions:
        print("No Claude session messages found for " + str(target) + ".")
        sys.exit(0)

    total_msgs    = sum(len(m) for _, m in sessions)
    merged_blocks = merge_overlapping_blocks(sessions)
    print("Sessions: " + str(len(sessions)) + " | Messages: " + str(total_msgs) +
          " | Blocks: " + str(len(merged_blocks)))

    # Pre-generate all descriptions
    print()
    auto_entries  = []
    skipped_short = 0
    ...
    for i, (start_utc, end_utc, all_msgs) in enumerate(merged_blocks, 1):
        ...
        if mins < MIN_BLOCK_MINUTES:
            skipped_short += 1
            continue
        print("  Describing block " + str(i) + "/" + str(len(merged_blocks)) + "...", end=" ", flush=True)
    ...
    if skipped_short:
        print("  (" + str(skipped_short) + " block(s) under " + str(MIN_BLOCK_MINUTES) + " min -- skipped)")

    # Show detected sessions table
    if auto_entries:
        print_day_table(auto_entries, LOCAL_UTC_OFFSET, "SESIONES DETECTADAS -- " + str(target))
    else:
        print("\n  No blocks detected for " + str(target) + ".")

    # Ask for manual entries
    manual_entries = ask_manual_entries(target, LOCAL_UTC_OFFSET)

    # Build complete list sorted by start time
    all_entries = sorted(auto_entries + manual_entries, key=lambda x: x["start_utc"])

    # Show complete day table if manual entries were added
    if manual_entries:
        print_day_table(all_entries, LOCAL_UTC_OFFSET, "PLAN COMPLETO DEL DIA -- " + str(target))
    elif not auto_entries:
        print("\nNothing to log.")
        sys.exit(0)

    # Get existing Clockify entries for duplicate detection
    ...
    print()
    print("-" * 60)
    print("  Confirmacion -- " + str(total) + " entrada" + ("s" if total != 1 else ""))
    print("-" * 60)
    ...
    for i, entry in enumerate(all_entries, 1):
        ...
        print("  [" + str(i) + "/" + str(total) + "]  " +
              sl.strftime("%H:%M") + "--" + el.strftime("%H:%M") +
              "  " + dur + "  " + src)
        ...
        if overlaps:
            print("  (!) DUPLICATE -- " + str(len(overlaps)) + " existing entr" +
                  ("y" if len(overlaps) == 1 else "ies") + " overlap:")
            for ov in overlaps:
                ...
                print("     * " + ov_s.strftime("%H:%M") + "--" + ov_e.strftime("%H:%M") +
                      "  '" + ov.get("description", "")[:55] + "'")

        if args.dry_run:
            tag = " (DUPLICATE RISK)" if overlaps else ""
            print("  -> [DRY RUN] Would create" + tag + ".")
            continue

        prompt_txt = "  Create anyway? [y/n/e=edit]: " if overlaps else "  Create? [y/n/e=edit]: "
        try:
        ...
        if ok:
            ...
            print("  ERROR: " + str(result)[:80])
            skipped += 1

    print()
    print("-" * 60)
    if args.dry_run:
        print("  DRY RUN -- " + str(len(all_entries)) + " entr" +
              ("y" if len(all_entries) == 1 else "ies") + " would be created.")
    else:
        print("  Done -- " + str(created) + " created, " + str(skipped) + " skipped.")
    print()
